...
class GeoTiff(object):
    ...
    def clip_with_grid(self, clip_size, begin_id, out_dir_mask, out_dir_tif):
        """
        clip mask and tif with grid
        Args:
            clip_size: int (should be divisible by 32 depending on ml architecture)
            out_dir: str
        ...
        if not os.path.exists(out_dir_mask):
            ...
            print('create dir', out_dir_mask)

        if not os.path.exists(out_dir_tif):
            # check the dir
            os.makedirs(out_dir_tif)
            print('create dir', out_dir_tif)
        ...
        for i in range(row_num):
            for j in range(col_num):
                clipped_mask = np.array(
                    self.mask[0, i * clip_size: (i + 1) * clip_size, j * clip_size: (j + 1) * clip_size])
                ins_list = np.unique(clipped_mask)
                ins_list = ins_list[1:]

                clipped_image = np.array(
                    self[i * clip_size: (i + 1) * clip_size, j * clip_size: (j + 1) * clip_size])
                clipped_image = clipped_image.astype(np.int8)

                for id in range(len(ins_list)):
                    bg_img = np.zeros(clipped_mask.shape).astype(np.int8)
                    if ins_list[id] > 0:
                        bg_img[np.where(clipped_mask == ins_list[id])] = 255
                    try:
                        mask_save_path = os.path.join(out_dir_mask,
                                                      '%d_%s_%d.tif' % (begin_id + i * col_num + j, 'img', id))
                        image_save_path = os.path.join(out_dir_tif,
                                                       '%d_%s_%d.tif' % (begin_id + i * col_num + j, 'img', id))
                        save_image_with_georef(bg_img, gtiffDriver,
                                               self.dataset, j * clip_size, i * clip_size, mask_save_path)
                        save_image_with_georef(clipped_image, gtiffDriver,
                                               self.dataset, j * clip_size, i * clip_size, image_save_path)
                        print('clipped mask & stamp successfully！(%d/%d)' % (count, row_num * col_num))
                        count += 1
                    except Exception:
        ...
        for i in range(row_num):
            for j in range(col_num):
                ...
                for id in range(len(ins_list)):
                    ...
                    try:
                        save_path = os.path.join(out_dir, '%d_%s_%d.tif' % (begin_id + i * col_num + j, 'msk', id))
                        save_image_with_georef(bg_img, gtiffDriver,
                                               self.dataset, j * clip_size, i * clip_size, save_path)
    ...
    def clip_tif_and_shapefile(self, clip_size, begin_id, shapefile_path, out_dir):
        ...
        pic_id = self.clip_tif_with_grid(clip_size=clip_size, begin_id=begin_id, out_dir=out_dir + '/stamps')
        """
        out_dir_mask = out_dir + '/annotations'
        out_dir_tif = out_dir + '/stamps'
        pic_id = self.clip_with_grid(clip_size, begin_id, out_dir_mask, out_dir_tif)
...
def save_image_with_georef(image, driver, original_ds, offset_x=0, offset_y=0, save_path=None):
    ...
    try:
        ...
        gdalnumeric.CopyDatasetInfo(original_ds, ds)
        gdalnumeric.CopyDatasetInfo(original_ds, ds, xoff=offset_x, yoff=offset_y)
