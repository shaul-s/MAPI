...
def copy_data_new(input_path, id, num, mark='train'):
    if num != 0:
        list = os.listdir(input_path + '/' + img_path)
        ann_list = os.listdir(input_path + '/' + ann_path)
        if not os.path.isdir(input_path + '/' + mark + '/' + img_path):
...
def slice(input_path, train=0.8, eval=0.2, test=0.0):
    ...
    copy_data_new(input_path, train_id, n_train, 'train')
    copy_data_new(input_path, eval_id, n_eval, 'eval')
    copy_data_new(input_path, test_id, n_test, 'test')
